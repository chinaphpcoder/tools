            ...
            <div class="col-md-12">
                <div class="box">
                    <div class="box-header">
                        ...
                        <a href="{{route('college_add')}}" class="btn btn-default" type="button">新增</a>
                        <a href="{{route('college_delete')}}" id="deleteBulk" class="btn btn-default" type="button">批量删除</a>
                    </div>
                        ...
                        <table id="example2" class="table table-bordered table-hover">
                            ...
                            @foreach ($list as $row)
                                <tr>
                                    ...
                                    <td align="center">
                                        @if($row->status == 1)
                                            <a class="btn btn-default" href="{{route('college_status')}}?id={{$row->id}}" role="button">下线</a>
                                        @elseif($row->status == 2)
                                            <a class="btn btn-default" href="{{route('college_status')}}?id={{$row->id}}" role="button">上线</a>
                                        @endif
                                        <a class="btn btn-default" href="{{route('college_edit')}}?id={{$row->id}}" role="button">编辑</a>
                                        <a class="btn btn-default row-delete" href="{{route('college_delete')}}?id={{$row->id}}" role="button">删除</a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
