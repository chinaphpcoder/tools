                ...
                <div class="col-md-12">
                    <div class="box">
                        <div class="box-header">
                            ...
                            <a href="{{route('pcbanner_add')}}" class="btn btn-default" type="button">新增</a>
                            <a href="{{route('pcbanner_delete')}}" id="deleteBulk" class="btn btn-default" type="button">批量删除</a>
                            <button type="button" class="btn btn-default" data-toggle="modal" data-target="#myModal">轮播图排序设置</button>
                            <a href="{{route('pcbanner_index')}}" id="deleteBulk" class="btn btn-default <?=(is_null($status) ? 'active' : '')?>" type="button">全部</a>
                            <a href="{{route('pcbanner_index', ['status' => 0])}}" id="deleteBulk" class="btn btn-default <?=($status === 0 ? 'active' : '')?>" type="button">待发布</a>
                            <a href="{{route('pcbanner_index', ['status' => 1])}}" id="deleteBulk" class="btn btn-default <?=($status === 1 ? 'active' : '')?>" type="button">已上线</a>
                            <a href="{{route('pcbanner_index', ['status' => 2])}}" id="deleteBulk" class="btn btn-default <?=($status === 2 ? 'active' : '')?>" type="button">已下线</a>
                        </div>
                            ...
                            <table id="example2" class="table table-bordered table-hover">
                                ...
                                @foreach ($list as $row)
                                    <tr>
                                        ...
                                        <td align="center">
                                            @if($row->status == 0)
                                                <a class="btn btn-default" href="{{route('pcbanner_status', ['id' => $row->id, 'status' => $status])}}" role="button">发布</a>
                                            @elseif($row->status == 2)
                                                <a class="btn btn-default" href="{{route('pcbanner_status', ['id' => $row->id, 'status' => $status])}}" role="button">上线</a>
                                            @else
                                                <a class="btn btn-default" href="{{route('pcbanner_status', ['id' => $row->id, 'status' => $status])}}" role="button">下线</a>
                                            @endif
                                            <a class="btn btn-default" href="{{route('pcbanner_edit', ['id' => $row->id])}}" role="button">编辑</a>
                                            <a class="btn btn-default row-delete" href="{{route('pcbanner_delete', ['id' => $row->id])}}" role="button">删除</a>
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
