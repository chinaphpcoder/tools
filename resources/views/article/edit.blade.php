...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{route('article_index')}}">H5公告管理</a></li>
        ...
        <div class="col-md-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <a href="{{route('article_index')}}" class="btn btn-default" type="button">返回列表</a>
                </div>
                <form id="form" method="POST" action="{{route('article_update')}}" role="form">
                    {{ csrf_field() }}
                    ...
                    <div class="box-body">
                        <div class="form-group {{ $errors->has('type_id') ? ' has-error' : '' }}">
                            ...
                            <select class="form-control" name="type_id">
                                ...
                                @foreach($type as $one)
                                    <option <?=($one->id == $row->type_id ? 'selected="selected"' : '')?> value="{{$one->id}}">{{$one->title}}</option>
                                @endforeach
                            </select>
