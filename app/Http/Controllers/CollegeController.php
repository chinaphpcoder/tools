...
class CollegeController extends Controller{
    ...
    public function index(){
        $list = College::orderBy('id', 'desc')->with('user', 'type')->paginate(20);
        ...
        $this->view_data['meta_title'] = '小沙学院列表';
        $this->view_data['list'] = $list;
        return view('college.index', $this->view_data);
    }
     ...
     */
    public function add(){
        $type = ArticleType::where('type_id', 1)->select('id', 'title')->get();
        $this->view_data['type'] = $type;
        $this->view_data['meta_title'] = '新增小沙学院';
        return view('college.add', $this->view_data);
    }
    ...
    public function edit(Request $request){
        ...
        //查询指定记录
        $row = College::where('id', $id)->first();
        if (empty($row)) {
            return $this->error('没有查询到指定记录');
        }
        
        $this->view_data['row'] = $row;
        $this->view_data['meta_title'] = '编辑小沙学院';
        return view('college.edit', $this->view_data);
    }
    ...
    public function update(Request $request){
        ...
        $college->content = $request->input('content');
        $college->type_id = $request->input('type_id');
        $college->status = $status;
        $college->user_id = Auth::id();

        if ($status == 1) {
            $college->published_at = date('Y-m-d H:i:s');
        }

        $result = $college->save();
        
        if ($result) {
            return $this->success(route('college_list'));
        }else{
    ...
    public function delete(Request $request){
        ...
        //批量删除
        $result = College::destroy($ids);
    
        if ($result) {
            return $this->success(route('college_list'), '删除成功');
        } else {
